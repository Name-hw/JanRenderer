...
fn addPkg_C(b : *std.Build, pkg_name : [] const u8,
  ...
  pkg_lib.addCSourceFiles(.{
      .root = pkg_dir_src, .files = src_files, .flags = &.{"-std=c99"}});
...
pub fn build(b : *std.Build) !void {
  ...
  const JrClasses_lib = b.addStaticLibrary(.{
      .name = "JrClasses",
      .root_source_file = b.path("src/JanRenderer/JrClasses"),
      .target = target,
      .optimize = optimize,
  });
  ...
  JanRenderer_lib.addCSourceFiles(.{
      .root = b.path(""), .files = CSourceFiles, .flags = CFlags});
  // JanRenderer_lib vcpkg library
  JanRenderer_lib.addIncludePath(
      b.path("vcpkg_installed/x64-windows/include/"));
  ...
  const JanRenderer_lib_install_artifact = b.addInstallArtifact(
      JanRenderer_lib, .{.dest_dir =.{.override =.{.custom = "../bin"}}});
  ...
  const exe_install_artifact = b.addInstallArtifact(
      TestApp_exe, .{.dest_dir =.{.override =.{.custom = "../bin"}}});
