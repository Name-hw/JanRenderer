...
fn addCglm(b: *std.Build, target: std.Build.ResolvedTarget, optimize: std.builtin.OptimizeMode) !*std.Build.Step.Compile {
    const cwd = fs.cwd();

    // cglm_dep
    const cglm_dep = b.dependency("cglm", .{});
    const cglm_dir = cglm_dep.path("");
    const cglm_dir_include = cglm_dir.path(b, "include");
    const cglm_dir_src = cglm_dir.path(b, "src");
    ...
    // cglm_lib
    const cglm_lib = b.addStaticLibrary(.{
        .name = "cglm",
        .target = target,
        .optimize = optimize,
    });
    cglm_lib.linkLibC();
    cglm_lib.addIncludePath(cglm_dir_include);
    cglm_lib.installHeadersDirectory(cglm_dir_include, "", .{});

    const src_files = try findCSourceFiles(b, absoluteDir_src);

    cglm_lib.addCSourceFiles(.{ .root = cglm_dir_src, .files = src_files, .flags = &.{"-std=c99"} });
...
pub fn build(b: *std.Build) !void {
    ...
    // cglm_lib
    const cglm_lib = try addCglm(b, target, optimize);

    // JrClasses_lib
    const JrClasses_lib = b.addStaticLibrary(.{
        .name = "JrClasses",
        .root_source_file = b.path("src/JanRenderer/JrCamera.zig"),
        .target = target,
        .optimize = optimize,
    });
    // JrClasses_lib C source
    JrClasses_lib.linkLibC();
    JrClasses_lib.linkLibrary(cglm_lib);
    ...
    // JanRenderer_lib
    const JanRenderer_lib = b.addSharedLibrary(.{
        .name = "JanRenderer",
        .target = target,
        .optimize = optimize,
    });
    ...
    JanRenderer_lib.linkLibrary(cglm_lib);
    JanRenderer_lib.linkLibrary(JrClasses_lib);
    //JanRenderer_lib.addIncludePath(.{ .cwd_relative = "C:/Users/user/AppData/Roaming/Code/User/globalStorage/ziglang.vscode-zig/zig/windows-x86_64-0.13.0/lib/" }); // JrClasses.h includes zig.h. (The path may change depending on who builds it.)
