...
pub export fn jrGui_init(self: *JrGui) callconv(.C) void {
    ...
    zgui.init(allocator);
    self.defaultFont = zgui.io.addFontFromFile("assets/fonts/Noto_Sans/NotoSans-VariableFont_wdth,wght.ttf", 16.0);
    self.style = zgui.getStyle();

    zgui.backend.init(self.initInfo, self.window);

    zgui.io.setDefaultFont(self.defaultFont);
}
